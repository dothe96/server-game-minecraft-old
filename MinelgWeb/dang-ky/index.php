...
<!--[if gt IE 8]><!--> <html lang="vi" prefix="og: http://ogp.me/ns# fb: http://ogp.me/ns/fb#" class="no-js"><!--<![endif]-->
	...
	<link href="../assetslg/css/bootstrap.css" rel="stylesheet">
		...
		<title>Đăng Ký - Server Minecraft Việt Nam - Minelg.com</title>
		...
		<div class="container1">
                <div class="menutong">
                    <ul class="menulg1"> <!--edit mac dinh-->
                        ...
                        <li class="macdinhlg"><a href="http://minelg.com/dang-ky"><i class="fa fa-user"></i> Đăng ký</a></li>
	...
	<div id="content">
		<div id="inner-lg" class="wrap2 clearfix">
			<div class="nentrangdad nentrang">
					<font size="5"><i class="fa fa-user"></i> Đăng ký</font>
			</div>
		</div>
	</div>
	<div id="content2">
...
<div id="main" class="ninecol first clearfix" role="main">
	<div class="inner-bg">
		...
		<h1><span style="color: #424242; font-weight: bold;"><div class="minimenu"><font color="#F2F2F2"></font> Đăng ký tài khoản</div></span></h1>
				<div class="bovien">
						<p style="text-align: left; color: red; font-size: 20px; font-weight: bold;"> Lưu ý khi đăng ký!</p>
<font color="black">►</font> Nhập đầy đủ thông tin tại các trường có "<font color="red">*</font>".</br><font color="black">►</font> Ghi nhớ Email khi đăng ký (Dùng Email để xác thực khi có vấn đề xảy ra).</br><font color="black">►</font> Độ dài tài khoản từ 4 » 16 ký tự. Độ dài mật khẩu từ 5 » 30 ký tự.</br><font color="black">►</font> Các tài khoản có tên không hợp lệ (Nhạy cảm, tên Server khác...etc) đều bị khóa.
<form method="post" enctype="multipart/form-data"><div class="login-form"><p style="width: 80%; background: #DF3A01; text-align: center; color: white; font-size: 18px; font-weight: bold; margin-bottom: -20px; margin-top: -20px; border-radius: 20px; margin-right: auto; margin-left: auto;">
...
?>
<?php //xử lý thông tin đầu vào
	 #1 Xử lý độ dài tài khoản mk
		if (isset($_POST['reg_submit']))
			{
				$dodaitk = strlen($_POST['reg_username']);
				$dodaimk = strlen($_POST['reg_password']);
					if ($dodaitk<4 or $dodaitk>16)
						{
							$kttk=1;
						}
					else
...
<?php
						if (isset($_POST['reg_submit']))
						{	#kiểm tra độ dài tk mk
							if ($kttk==1 or $ktmk==1)
								{
									echo "Lỗi : Độ dài tài khoản hoặc mật khẩu không đúng!";
								}
							else
								{	#kiểm tra khoảng trống
									if (strpos($_POST['reg_username'],' ')!==false or strpos($_POST['reg_password'],' ')!==false or strpos($_POST['reg_email'],' ')!==false)
									{
										echo "Lỗi: các trường không được chứa khoảng trống!";							
									}
									else
										{	#kiểm tra ký tự tk
											$truongsaitk = '/[^a-zA-Z0-9]/';
											if (preg_match($truongsaitk, $_POST['reg_username'])==true){
												echo "Lỗi: tài khoản chứa ký tự không hợp lệ!";
											}
											else{	#kiểm tra ký tự email
													if (!eregi("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$", $_POST['reg_email']))
    													{
       														echo "Lỗi: Email không hợp lệ. Vui lòng nhập lại!";
    													}
													else{	#kiểm tra tồn tại
															$tentk = $_POST['reg_username'];
															if (mysql_num_rows(mysql_query("SELECT username FROM thanhvien WHERE username='$tentk'")) > 0){
																echo "Lỗi: Tài khoản đã tồn tại!";
															}
															else {
																...
																else { //lưu thông tin vào MySQL
																	$matkhau = $_POST['reg_password'];
...
?>
	</p></br><font style="font-weight: bold;">Tài khoản:</font><font style="font-weight: bold; color: red;">*</font>
	<div class="form-group __web-inspector-hide-shortcut__"><input name="reg_username" type="text" title="" value="" class="form-control login-field" id="reg-username" placeholder="Nhập tài khoản" required="required">
		<label class="login-field-icon fui-user" for="reg-username"></label>
	</div>
	<font style="font-weight: bold;">Mật khẩu:</font><font style="font-weight: bold; color: red;">*</font>
	<div class="form-group"><input name="reg_password" type="password" title="" value="" class="form-control login-field" id="reg-password" placeholder="Nhập mật khẩu" required="required">
		<label class="login-field-icon fui-lock" for="reg-password"></label>
	</div>
	<font style="font-weight: bold;">Email: (Dùng để lấy lại mật khẩu hoặc đổi mật khẩu)</font><font style="font-weight: bold; color: red;">*</font>
	<div class="form-group"><input name="reg_email" type="email" title="" value="" class="form-control login-field" id="reg-email" placeholder="Nhập địa chỉ Email" required="required">
		<label class="login-field-icon fui-mail" for="reg-email"></label>
	</div><br>
	<div class="form-group"><input type="submit" name="reg_submit" title="" value="Đăng Ký" id="submit-button" class="btn btn-primary btn-lg btn-block"></div>
	</div>
</form>
<p style="text-align: center; color: #04B45F; font-size: 20px; font-weight: bold;">Thay đổi mật khẩu - Quên mật khẩu</p>
						<div class="taive2">
							<a href="http://minelg.com/quen-mat-khau" style="text-decoration: none;"><div class="tvj1">Quên mật khẩu</div></a><a href="http://minelg.com/doi-mat-khau" style="text-decoration: none;"><div class="tvj2">Đổi mật khẩu</div></a>
						</div>
						<p style="height: 30px;"></p>
<p style="text-align: center; color: #04B45F; font-size: 20px; font-weight: bold;">Chính sách bảo mật</p>
<p style="text-align: justify; color: #2A0A12; font-size: 15px; margin: 0px 20px 5px 20px;">Để bảo mật tốt nhất cho người chơi, server minecraft legend thực hiện mã hóa mật khẩu PasswordHash (Mã băm) md5. Tất cả các thông tin liên quan tới mật khẩu người chơi cung cấp trong quá trình đăng ký đều được bảo mật và chỉ người chơi biết. Vì server không can thiệp tới quyền bảo mật về mật khẩu người chơi nên phải giải quyết qua Email. Email là bắt buộc trong những trường hợp liên quan tới tài khoản như, quên hoặc mất mật khẩu, thay đổi tài khoản...etc. Xác thực qua tin nhắn gửi tới Email đăng ký nên hãy chắc chắn rằng nó chính chủ và đang sử dụng. Thân!</p>
				</div><!--bovien-->
				<p></p>
