...
<html lang="vi" prefix="og: http://ogp.me/ns# fb: http://ogp.me/ns/fb#" class="no-js">
	...
	<link href="../assetslg/css/bootstrap.css" rel="stylesheet">
		...
		<title>Quên mật khẩu - Lấy lại mật khẩu - Server Minecraft Việt Nam - Minelg.com</title>
	...
	<div id="content">
		<div id="inner-lg" class="wrap2 clearfix">
			<div class="nentrangdad nentrang">
					<font size="5"><i class="fa fa-user"></i> Quên mật khẩu</font>
			</div>
		</div>
	</div>
	<div id="content2">
...
<div id="main" class="ninecol first clearfix" role="main">
	<div class="inner-bg">
		...
		<h1><span style="color: #424242; font-weight: bold;"><div class="minimenu"><font color="#F2F2F2"></font> Lấy lại mật khẩu</div></span></h1>
				<div class="bovien">
						<p style="text-align: justify; color: #b9136f; font-size: 17px; font-weight: bold; padding: 10px 10px 0px;">► Nếu bạn quên mật khẩu bạn có thể lấy lại mật khẩu thông qua email.<br>► Nhập vào tên tài khoản và email, sau đó mật khẩu sẽ được gửi tới email của bạn.<br>► Server không giải quyết trường hợp không có email.</p>
						<p></p>
				...
				<center><?php 
					if (isset($_POST['submit'])){
						$tentk = $_POST['usernamext'];
						if (mysql_num_rows(mysql_query("SELECT username FROM thanhvien WHERE username='$tentk'")) > 0){
							$emailsskt = $_POST['email'];
							if(mysql_num_rows(mysql_query("SELECT email FROM thanhvien WHERE email='$emailsskt'")) > 0){
								$mk = mysql_fetch_array(mysql_query("SELECT matkhau FROM chuamahoatk WHERE taikhoan='$tentk'"));
								//goi thu vien
							    include('class.smtp.php');
							    include "class.phpmailer.php"; 
							    $nFrom = "Minelg.com";    //mail duoc gui tu dau, thuong de ten cong ty ban
							    $mFrom = 'hotro.minelg@gmail.com';  //dia chi email cua ban 
							    $mPass = 'anhthetq96!!!';       //mat khau email cua ban
							    $nTo = $_POST['username']; //Ten nguoi nhan
							    $mTo = $_POST['email'];   //dia chi nhan mail
							    $mail             = new PHPMailer();
							    $title = 'MINELG.COM: Cấp lại mật khẩu';   //Tieu de gui mail
							    $body  = 'Xin chào '.$_POST['usernamext'].'!<br><br>Tài khoản của bạn vừa được kích hoạt cấp lại mật khẩu Server Minelg.com!<br><br>Mật khẩu của bạn: '.$mk['matkhau'].'<br><br>Lưu ý: Đây là email tự động vui lòng không reply<br>Mọi thắc mắc liên hệ facebook: https://facebook.com/groups/minelg<br><br>Trân trọng<br>Server Minecraft Việt Nam - Minelg.com<br>Owner: Đỗ Thế';   // Noi dung email
							    $mail->IsSMTP();             
							    $mail->CharSet  = "utf-8";
							    ...
							    $mail->AddReplyTo('hotro.minelg@gmail.com', 'Minelg.com'); //khi nguoi dung phan hoi se duoc gui den email nay
							    $mail->Subject    = $title;// tieu de email 
							    $mail->MsgHTML($body);// noi dung chinh cua mail se nam o day.
							    $mail->AddAddress($mTo, $nTo);;   
								if(!$mail->Send())   
								{   
								    echo "Lỗi: không thể gửi! có thể email không hoạt động";   
								}   
								else   
								{   
								    echo '<p style="text-align: center; font-size: 18px; font-weight: bold; margin-bottom: -30px;">Mật khẩu đã được gửi tới email của bạn<br>Kiểm tra trong email!</p><br>';
								} #else send
							}#if kt email
							else{
				...
				<form method="post" enctype="multipart/form-data"><div class="login-form">
						<font style="font-weight: bold;">Tài khoản:</font><font style="font-weight: bold; color: red;">*</font>
						<div class="form-group __web-inspector-hide-shortcut__"><input name="usernamext" type="text" title="" value="" class="form-control login-field" id="reg-username" placeholder="Nhập tài khoản" required="required">
							<label class="login-field-icon fui-user" for="reg-username"></label>
						</div>
						<font style="font-weight: bold;">Email: (Email khi đăng ký)</font><font style="font-weight: bold; color: red;">*</font>
						<div class="form-group"><input name="email" type="email" title="" value="" class="form-control login-field" id="reg-email" placeholder="Nhập địa chỉ Email" required="required">
							<label class="login-field-icon fui-mail" for="reg-email"></label>
						</div>
						<div class="form-group"><input type="submit" name="submit" title="" value="Cấp Lại Mật Khẩu" id="submit-button" class="btn btn-primary btn-lg btn-block"></div>
						</div>
				</form>
				</div><!--bovien-->
				<p></p>
