...
<html lang="vi" prefix="og: http://ogp.me/ns# fb: http://ogp.me/ns/fb#" class="no-js">
	...
	<link href="../assetslg/css/bootstrap.css" rel="stylesheet">
		...
		<title>Đổi Mật Khẩu - Server Minecraft Việt Nam - Minelg.com</title>
	...
	<div id="content">
		<div id="inner-lg" class="wrap2 clearfix">
			<div class="nentrangdad nentrang">
					<font size="5"><i class="fa fa-cogs"></i> Đổi Mật Khẩu</font>
			</div>
		</div>
	</div>
	<div id="content2">
...
<div id="main" class="ninecol first clearfix" role="main">
	<div class="inner-bg">
		...
		<h1><span style="color: #424242; font-weight: bold;"><div class="minimenu"><font color="#F2F2F2"></font> Change Password</div></span></h1>
				<div class="bovien">
						

				<p style="text-align: center; color: red; font-size: 22px; font-family: cursive; font-weight: bold;">Điều khoản đổi mật khẩu</p>
				<p style="text-align: justify-all;">
					► Server Minelg yêu cầu người dùng cung cấp Email chính xác và còn sử dụng trong quá trình đăng ký! <br>
					►Tất cả thủ tục hoàn toàn tự động, nên Server sẽ không giải quyết khi người dùng không thể xác thực qua Email hoặc quên Email. <br>
					► Nên đặt mật khẩu có ký tự số, ký tự đặc biệt... để đảm bảo cho tài khoản! <br>
				</p>

				<form method="post" enctype="multipart/form-data"><div class="login-form">
				<p style="width: 80%; background: #DF3A01; text-align: center; color: white; font-size: 18px; font-weight: bold; margin-bottom: -20px; margin-top: -20px; border-radius: 20px; margin-right: auto; margin-left: auto;">
					...
					<?php 
						if (isset($_POST['changesubmit'])){
							...
							$updatemk = $_POST['newpassword'];
							if (mysql_num_rows(mysql_query("SELECT username FROM thanhvien WHERE username='$tentk'")) > 0){
								$mkss = $_POST['oldpassword'];
								if(mysql_num_rows(mysql_query("SELECT matkhau FROM chuamahoatk WHERE matkhau='$mkss'")) > 0){
									if(strpos($updatemk,' ')==false){
										$dodaimk = strlen($updatemk);
										if($dodaimk>=5 and $dodaimk<=30){
											$emailss = $_POST['checkemail'];
											if(mysql_num_rows(mysql_query("SELECT email FROM thanhvien WHERE email='$emailss'")) > 0){
												$mahoamk = md5($_POST['newpassword']);
												@$update1 = mysql_query("
													UPDATE thanhvien SET password='$mahoamk' WHERE username='$tentk'
													");
												@$update2 = mysql_query("
													UPDATE chuamahoatk SET matkhau='$updatemk' WHERE taikhoan='$tentk'
													");
												if($update1){
													if($update2){
												...
												echo "Lỗi: Email không chính xác!";
											}
										}
										else{
											echo "Lỗi: độ dài mật khẩu từ 5 -> 30 ký tự!";
										}
									}
									else{
										echo "Lỗi: mật khẩu không được chứa khoảng trống!";
									}
								}
								else{
				...
				<div class="form-group __web-inspector-hide-shortcut__"><input name="username" type="text" title="" value="" class="form-control login-field" id="reg-username" placeholder="Nhập tài khoản" required="required">
				...
				<div class="form-group"><input name="oldpassword" type="password" title="" value="" class="form-control login-field" id="reg-password" placeholder="Nhập mật khẩu" required="required">
					<label class="login-field-icon fui-lock" for="reg-password"></label>
				</div>
				<font style="font-weight: bold;">Mật khẩu mới:</font><font style="font-weight: bold; color: red;">*</font>
				<div class="form-group"><input name="newpassword" type="password" title="" value="" class="form-control login-field" id="reg-password" placeholder="Nhập mật khẩu mới" required="required">
					<label class="login-field-icon fui-lock" for="reg-password"></label>
				</div>
				<font style="font-weight: bold;">Email:</font><font style="font-weight: bold; color: red;">*</font>
				<div class="form-group"><input name="checkemail" type="email" title="" value="" class="form-control login-field" id="reg-email" placeholder="Nhập địa chỉ Email" required="required">
					<label class="login-field-icon fui-mail" for="reg-email"></label>
				</div><br>
				<div class="form-group"><input type="submit" name="changesubmit" title="" value="Đổi Mật Khẩu" id="submit-button" class="btn btn-primary btn-lg btn-block"></div>
				</div>
				</form>
